...
----------------------------*/
	...
	if($action == 'ajax'){
		...
		$estatus_pendiente=1;
		$consulta="select count(id_banco) as numrows from bancos order by id_banco desc ";
		// echo $consulta;
		...
		$paginas="SELECT *, DATE_FORMAT(now(),'%m/%d/%Y') as hoy, DATE_FORMAT(date_add(now(),interval (365) day),'%m/%d/%Y') AS unano FROM bancos, entidades WHERE codigo_banco=bnkcd ORDER BY id_banco LIMIT $offset,$per_page ";
...
<!-- fin busqueda -->
		<table class="table table-striped table-bordered table-hover" id="dataTables-example">
			...
			while($row = $con->fetch(PDO::FETCH_ASSOC)){
				?>
				<tr>
					<td><?php echo substr($row['bankt'],0,30);?></td>
					...
					<td>
						<button type="button" class="btn btn-info" data-toggle="modal" data-target="#dataUpdate" data-id = "<?php echo $row['id_banco']?>" data-nombre = "<?php echo $row['nombre']?>" data-cuenta = "<?php echo $row['cuenta']?>" <i class='glyphicon glyphicon-edit'></i> Modificar</button>

						<button type="button" class="btn btn-warning" data-toggle="modal" data-target="#dataStatus" data-id="<?php echo $row['id_banco']?>" data-status="<?php echo $row['activo']?>" data-nombre="<?php echo $row['nombre']?>"  data-cuenta="<?php echo $row['cuenta']?>"><i class='glyphicon glyphicon-refresh '></i> Cambiar Status</button>

						<button type="button" class="btn btn-danger" data-toggle="modal" data-target="#dataDelete" data-id="<?php echo $row['id_banco']?>" data-status="<?php echo $row['activo']?>" data-nombre="<?php echo $row['nombre']?>" data-cuenta="<?php echo $row['cuenta']?>" ><i class='glyphicon glyphicon-trash'></i> Eliminar</button>
					</td>
						<?php 
						?>
					</td>
