...
if (empty($_POST['nombre']) or (empty($_POST['alias'])) or (empty($_POST['rif'])))
{
...
//		$final=explode('/', $final); $final=$final[2].'-'.$final[0].'-'.$final[1];
		...
		if ($con->rowCount() < 1)
		{
			$sql="INSERT INTO empresas (rif, nombre, alias, direccion1, direccion2, telefono, celular1, celular2, email, contacto, telefono_contacto, contacto_adm, telefono_contacto_adm, email_contacto, email_adm, activo, registrado, ip_registro, estado) VALUES (:rif, :nombre, :alias, :direccion1, :direccion2, :telefono, :celular1, :celular2, :email, :contacto, :telefono_contacto, :contacto_adm, :telefono_contacto_adm, :email_contacto, :email_adm, :activo, :registrado, :ip_registro, :estado)";
			$con=$db_con->prepare($sql);
			$uno=1;
			$query_update = $con->execute(array(
						":rif"=>$rif,
