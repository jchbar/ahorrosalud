...
----------------------------*/
	...
	if($action == 'ajax'){
		...
		$estatus_pendiente=1;
		$consulta="select count(id_especialidad) as numrows from especialidades order by nombre ";
		// echo $consulta;
		...
		$paginas="SELECT *, DATE_FORMAT(now(),'%m/%d/%Y') as hoy, DATE_FORMAT(date_add(now(),interval (365) day),'%m/%d/%Y') AS unano FROM especialidades ORDER BY id_especialidad LIMIT $offset,$per_page ";
		...
		<table class="table table-striped table-bordered table-hover" id="dataTables-example">
			...
			while($row = $con->fetch(PDO::FETCH_ASSOC)){
				...
				<tr>
					...
					<td>
						<button type="button" class="btn btn-info" data-toggle="modal" data-target="#dataUpdate" data-id="<?php echo $row['id_especialidad']?>" data-nombre="<?php echo $row['nombre']?>" ><i class='glyphicon glyphicon-edit'></i> Modificar</button>

						<button type="button" class="btn btn-warning" data-toggle="modal" data-target="#dataStatus" data-id="<?php echo $row['id_especialidad']?>" data-status="<?php echo $row['activo']?>" data-nombre="<?php echo $row['nombre']?>"><i class='glyphicon glyphicon-refresh '></i> Cambiar Status</button>

						<button type="button" class="btn btn-danger" data-toggle="modal" data-target="#dataDelete" data-id="<?php echo $row['id_especialidad']?>" data-status="<?php echo $row['activo']?>" data-nombre="<?php echo $row['nombre']?>" ><i class='glyphicon glyphicon-trash'></i> Eliminar</button>
					</td>
						<?php 
						?>
					</td>
