function load(page)
{
	...
	$("#loader").fadeIn('slow');
	$.ajax({
		url:'js_especialistas/lasespecialistas.php',
		data: parametros,
...
*/
		  var modal = $(this)
		  modal.find('.modal-title').text('Registro de Especialista' )
...
$( "#guardarDatos" ).submit(function( event ) { // incluir
	  ...
	  console.log(parametros);
			 $.ajax({
					type: "POST",
					url: "js_especialistas/agregar_especialista.php",
					data: parametros,
					beforeSend: function(objeto){
...
$('#dataUpdate').on('show.bs.modal', function (event) {
		  ...
		  var codigo_especialista = button.data('codigo_especialista')
		  ...
		  var modal = $(this)
		  modal.find('.modal-title').text('Modificar Datos Especialista : '+id +' '+nombre)
		  modal.find('.modal-body #id').val(id)
		  modal.find('.modal-body #rif').val(rif)
		  modal.find('.modal-body #nombre').val(nombre)
		  modal.find('.modal-body #codigo_especialidad').val(codigo_especialista)
...
$( "#actualizarDatos" ).submit(function( event ) { // modificar
		var parametros = $(this).serialize();
			 $.ajax({
					type: "POST",
					url: "js_especialistas/modificar_especialista.php",
					data: parametros,
					 beforeSend: function(objeto){
...
$('#dataStatus').on('show.bs.modal', function (event) {
		  ...
		  modal.find('.modal-title').text('Cambiar Status Especialista ' +id + ' / '+nombre+ ' de '+ estadoA +' -> '+estadoN)
		  /*
...
$( "#actualizarStatus" ).submit(function( event ) { // modificar
		...
		// console.log(parametros)
			 $.ajax({
					type: "POST",
					url: "js_especialistas/cambiar_especialista.php",
					data: parametros,
					 beforeSend: function(objeto){
...
$('#dataDelete').on('show.bs.modal', function (event) {
	...
	modal.find('.modal-body #nombre').val(nombre) 
	modal.find('.modal-body #rif').text(rif)
	modal.find('.modal-title').text('Eliminar Especialista' +id + ' / '+rif + ' / '+nombre)
})
$( "#eliminarDatos" ).submit(function( event ) {
	var parametros = $(this).serialize();
	$.ajax({
		type: "POST",
		url: "js_especialistas/eliminar_especialista.php",
		data: parametros,
		beforeSend: function(objeto){
