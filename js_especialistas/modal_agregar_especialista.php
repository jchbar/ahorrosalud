		    	...
		    	<div class="modal-header">
		      	...
		      	<div class="modal-body">
					<div class="form-group form-inline col-xs-12">
						...
						<label for="codigo_especialidad" class="sr-only control-label">Especialidad </label>
						<?php
								include_once('../dbconfig.php');
								$comando="select * from especialidades order by nombre";
								$con=$db_con->prepare($comando);
								$con->execute();
								echo '<select class="form-control" name="codigo_especialidad" id="codigo_especialidad" size="1">';
								while($row = $con->fetch(PDO::FETCH_ASSOC))
									echo '<option '.$row['id_especialidad'].' value="'.$row['id_especialidad'].'">'.$row['nombre'].' </option>'; 
								echo '</select>'; 
						?>
					</div>
					<div class="form-group form-inline col-xs-12">
