function load(page)
{
	...
	$("#loader").fadeIn('slow');
	$.ajax({
		url:'js_promotors/laspromotors.php',
		data: parametros,
...
*/
		  var modal = $(this)
		  modal.find('.modal-title').text('Registro de Promotor' )
...
$( "#guardarDatos" ).submit(function( event ) { // incluir
	  ...
	  console.log(parametros);
			 $.ajax({
					type: "POST",
					url: "js_promotors/agregar_promotor.php",
					data: parametros,
					beforeSend: function(objeto){
...
$('#dataUpdate').on('show.bs.modal', function (event) {
		  ...
		  var codigo_promotor = button.data('codigo_promotor')
		  ...
		  var modal = $(this)
		  modal.find('.modal-title').text('Modificar Datos Promotor : '+id +' '+nombre)
		  modal.find('.modal-body #id').val(id)
		  modal.find('.modal-body #rif').val(rif)
		  modal.find('.modal-body #nombre').val(nombre)
		  modal.find('.modal-body #codigo_especialidad').val(codigo_promotor)
...
$( "#actualizarDatos" ).submit(function( event ) { // modificar
		var parametros = $(this).serialize();
			 $.ajax({
					type: "POST",
					url: "js_promotors/modificar_promotor.php",
					data: parametros,
					 beforeSend: function(objeto){
...
$('#dataStatus').on('show.bs.modal', function (event) {
		  ...
		  modal.find('.modal-title').text('Cambiar Status Promotor ' +id + ' / '+nombre+ ' de '+ estadoA +' -> '+estadoN)
		  /*
...
$( "#actualizarStatus" ).submit(function( event ) { // modificar
		...
		// console.log(parametros)
			 $.ajax({
					type: "POST",
					url: "js_promotors/cambiar_promotor.php",
					data: parametros,
					 beforeSend: function(objeto){
...
$('#dataDelete').on('show.bs.modal', function (event) {
	...
	modal.find('.modal-body #nombre').val(nombre) 
	modal.find('.modal-body #rif').text(rif)
	modal.find('.modal-title').text('Eliminar Promotor' +id + ' / '+rif + ' / '+nombre)
})
$( "#eliminarDatos" ).submit(function( event ) {
	var parametros = $(this).serialize();
	$.ajax({
		type: "POST",
		url: "js_promotors/eliminar_promotor.php",
		data: parametros,
		beforeSend: function(objeto){
