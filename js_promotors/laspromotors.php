...
----------------------------*/
	...
	if($action == 'ajax'){
		...
		$estatus_pendiente=1;
		$consulta="select count(id_promotor) as numrows from promotores ";
		// echo $consulta;
		...
		$paginas="SELECT *, DATE_FORMAT(now(),'%m/%d/%Y') as hoy, DATE_FORMAT(date_add(now(),interval (365) day),'%m/%d/%Y') AS unano FROM promotores ORDER BY nombre LIMIT $offset,$per_page ";
					...
					<script type="text/javascript">
					    $(document).ready(function()
					    {
					        ...
					        $("#busqueda").keyup(function(e){
					              ...
					              $.ajax({
					                    type: "POST",
								url: "js_promotors/php/search.php",
								data: { query: consulta },
					                    dataType: "html",
					                    beforeSend: function(){
					                    //imagen de carga
		...
		<table class="table table-striped table-bordered table-hover" id="dataTables-example">
			...
			while($row = $con->fetch(PDO::FETCH_ASSOC)){
				...
				<tr>
					...
					<td>
					<?php
						echo '<button type="button" class="btn btn-info" data-toggle="modal" data-target="#dataUpdate" data-id = "'.$row['id_promotor'].'" data-nombre = "'.$row['nombre'].'" data-rif = "'.$row['rif'].'"  data-direccion1 = "'.$row['direccion1'].'" data-direccion2 = "'.$row['direccion2'].'" data-telefono = "'.$row['telefono'].'" data-celular1 = "'.$row['celular1'].'" data-celular2 = "'.$row['celular2'].'" data-email = "'.$row['email'].'"> <i class="glyphicon glyphicon-edit"></i> Modificar</button>';

						echo '<button type="button" class="btn btn-warning" data-toggle="modal" data-target="#dataStatus" data-id="'.$row['id_promotor'].'" data-status="'.$row['activo'].'" data-nombre="'.$row['nombre'].'"  data-rif="'.$row['rif'].'"><i class="glyphicon glyphicon-refresh "></i> Cambiar Status</button>';

						echo '<button type="button" class="btn btn-danger" data-toggle="modal" data-target="#dataDelete" data-id="'.$row['id_promotor'].'" data-status="'.$row['activo'].'" data-nombre="'.$row['nombre'].'" data-rif="'.$row['rif'].'" ><i class="glyphicon glyphicon-trash"></i> Eliminar</button>';
						?>
					</td>
						<?php 
						?>
